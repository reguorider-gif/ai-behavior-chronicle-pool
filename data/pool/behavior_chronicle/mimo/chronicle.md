...
- 继续观察：下一轮必须解释集中仓位的边际优势，并给出组合层面最大损失。
本轮输出必须说明：哪些历史经验被采用，哪些被拒绝，以及原因。

## Lessons

- [observe] 继续观察：下一轮必须显式评估平局基准概率，避免把平局系统性低估。（pattern=draw_avoidance, confidence=0.72）
- [observe] 继续观察：保留 no-bet 合法性，但必须说明信息缺口和机会成本。（pattern=uncertainty_to_no_bet, confidence=0.72）
- [observe] 继续观察：下一轮必须解释集中仓位的边际优势，并给出组合层面最大损失。（pattern=stake_concentration, confidence=0.675）
